...
This is the launch-safety belt for every Phylanx service. Importing this
module from an entrypoint asserts the configured Solana network is NOT
mainnet, unless `PHYLANX_MAINNET_OK` is explicitly set to `"1"`.
...
mean Phylanx can never run in production. We want every mainnet start to
...
In tests, this is a no-op — `PHYLANX_NETWORK` is unset, refusal only
fires when an entrypoint sets `PHYLANX_NETWORK=mainnet-beta` without
also setting `PHYLANX_MAINNET_OK=1`.
...
----------------------
  ...
  mainnet-beta   — production (refused unless PHYLANX_MAINNET_OK=1)
...
logger = logging.getLogger("phylanx.network_guard")
...
ENV_NETWORK    = "PHYLANX_NETWORK"
ENV_MAINNET_OK = "PHYLANX_MAINNET_OK"

# The default network if PHYLANX_NETWORK is unset. We default to LOCALNET
...
class UnsupportedNetwork(RuntimeError):
    """Raised when PHYLANX_NETWORK is set to an unrecognised value."""
...
def opted_in_to_mainnet() -> bool:
    """True iff PHYLANX_MAINNET_OK is set to `"1"`."""
...
def enforce_network_guard(*, service: str | None = None) -> NetworkVerdict:
    ...
    if verdict.is_production:
        ...
        logger.warning(
            ...
            "network %s with explicit PHYLANX_MAINNET_OK=1 opt-in",
