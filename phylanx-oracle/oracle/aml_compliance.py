...
--------------------------------------
    "Large-scale AI agent lending enabled by Phylanx certs may
...
The audit-flagged risk is NOT that Phylanx itself is an MSB / VASP /
...
The risk is the SHAPE of an adversarial regulatory complaint:
  ...
  * A consumer protocol uses a Phylanx cert to size a loan to an AI
  ...
  * `aml_program_attestation` — drawn from a closed enum
    ...
    time, ONE of:
      ...
      - `NO_AML_PROGRAM_REQUIRED_FOR_PHYLANX_ACTIVITY` — the
        ...
        the operator's activity *as a Phylanx cluster operator*
      ...
      - `EXTERNAL_AML_PROGRAM_DECLARED` — the operator runs an
        ...
        separate product). Their Phylanx cluster activity remains
        ...
        extend to their Phylanx capacity. This value is the same
...
Phylanx as an investment adviser. It does NOT implement SAR filing
...
class AmlProgramAttestation(str, Enum):
    ...
    The audit-mandated reading:
      ...
      * `NO_AML_PROGRAM_REQUIRED_FOR_PHYLANX_ACTIVITY` — the
        ...
        operator's Phylanx cluster activity does NOT trigger
      ...
      * `EXTERNAL_AML_PROGRAM_DECLARED` — the operator runs an AML
        ...
        Their Phylanx cluster activity is *outside* that
    ...
    that the operator's Phylanx activity is not a covered activity.
    ...
    gate refuses):
      ...
      * `PHYLANX_OPERATES_AS_MSB` / `PHYLANX_OPERATES_AS_VASP` etc.
...
#: Mirrored byte-for-byte in `phylanx-sdk/src/safe_reader.ts` (the
