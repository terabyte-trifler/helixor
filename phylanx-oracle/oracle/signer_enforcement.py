...
network. An explicit opt-in (`PHYLANX_INPROCESS_SIGNER_OK=1`) bypasses
...
logger = logging.getLogger("phylanx.oracle.signer_enforcement")
...
ENV_INPROCESS_SIGNER_OK = "PHYLANX_INPROCESS_SIGNER_OK"
...
class SignerEnforcementReport:
    ...
    `opted_in`            True iff `PHYLANX_INPROCESS_SIGNER_OK == "1"`.
...
def opted_in_to_inprocess_signer() -> bool:
    """True iff `PHYLANX_INPROCESS_SIGNER_OK` is set to `"1"`."""
...
def enforce_production_signer(
    ...
    *,
    ...
    Reads `PHYLANX_NETWORK` + `PHYLANX_INPROCESS_SIGNER_OK` from the
