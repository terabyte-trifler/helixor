...
The audit-flagged risk is structural, not implementation-bug shaped:
  ...
  * Cert scores are consumed by external lending markets to size
    loans. A consumer who reads a Phylanx cert and lends against an
...
--------------------------
It does NOT register Phylanx (or any operator) as an investment
...
class CompensationModel(str, Enum):
    ...
    refuses):
      ...
      * Equity in a Phylanx-affiliated entity that does not exist
...
#: `phylanx-sdk/src/safe_reader.ts` (the `ADVISORY_DISCLAIMER` export)
