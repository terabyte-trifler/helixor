...
running `verify_score_computation`:
  ...
  1. Clones phylanx at the published release tag.
...
`path` is the REPO-RELATIVE path from phylanx-oracle/ (e.g.
...
domain-tag prefix `phylanx-scoring-bundle-v1` reserves room for a v2
...
# Paths are repo-relative from `phylanx-oracle/`. Order does NOT matter for
...
_BUNDLE_DOMAIN_TAG = b"phylanx-scoring-bundle-v1\n"
...
def _repo_root() -> Path:
    """
    Return the `phylanx-oracle/` directory — the root the bundle paths
    are relative to. This module lives at
    `phylanx-oracle/scoring/bundle_hash.py`; parent.parent is
    `phylanx-oracle/`.
    """
...
def compute_scoring_bundle_hash(
    *,
    ...
    `repo_root` defaults to the `phylanx-oracle/` directory containing
