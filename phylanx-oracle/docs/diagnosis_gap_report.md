# Phylanx Diagnosis — Gap Report (Day 33, frozen)
...
|---|---|---|
| `GET /agents/{wallet}/health/{epoch}` (`phylanx-api/api/schemas.py::HealthResponse`) | `score`, `alert_tier`, `flag_set_token` (VULN-24 opaque hash), `flag_count` (popcount only) | **score-only** |
...
### What's in phylanx-oracle (off-chain)
...
| API endpoint `GET …/diagnosis/{epoch}`  | **Open — Day 34** | New `DiagnosisResponse` schema in `phylanx-api/api/schemas.py`. |
