...
adversarial complaint lands. Phylanx itself is not an MSB / VASP /
...
AML-1 closes the substrate path by:
  * Extending `OperatorAttestation` with `aml_program_attestation`
    (closed enum: NO_AML_PROGRAM_REQUIRED_FOR_PHYLANX_ACTIVITY |
...
_NO_AML = AmlProgramAttestation.NO_AML_PROGRAM_REQUIRED_FOR_PHYLANX_ACTIVITY.value
