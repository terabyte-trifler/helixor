...
REQUIREMENTS
------------
This test needs the program deployed at PHYLANX_PROGRAM_ID on the validator,
...
Environment:
    PHYLANX_INTEGRATION    = 1            opt-in flag
    PHYLANX_PROGRAM_ID                    deployed program ID
    SOLANA_RPC_URL                        e.g. http://127.0.0.1:8899
    ORACLE_KEYPAIR_PATH                   the oracle authority keypair
    PHYLANX_TEST_AGENT_PUBKEY             a registered, v2-layout agent
"""
...
INTEGRATION = os.environ.get("PHYLANX_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="set PHYLANX_INTEGRATION=1 + the validator env vars to run",
)
...
async def test_commit_baseline_round_trip_onchain():
    ...
    agent_wallet = os.environ["PHYLANX_TEST_AGENT_PUBKEY"]
