...
Pins the safety belt that prevents any Phylanx entrypoint from starting
...
def test_oracle_node_refuses_production_plaintext_grpc(monkeypatch):
    ...
    monkeypatch.setenv("PHYLANX_NETWORK", NETWORK_MAINNET_BETA)
    monkeypatch.setenv("PHYLANX_MAINNET_OK", "1")
    monkeypatch.delenv("PHYLANX_GRPC_TLS_CERT", raising=False)
    monkeypatch.delenv("PHYLANX_GRPC_TLS_KEY", raising=False)
    monkeypatch.delenv("PHYLANX_GRPC_TLS_CA_CERT", raising=False)
...
class TestMainnetRefusal:
    ...
    def test_mainnet_without_opt_in_is_refused(self):
        ...
        with override_network(NETWORK_MAINNET_BETA, mainnet_ok=False):
            ...
            assert "PHYLANX_MAINNET_OK" in str(exc.value)
...
class TestOverrideRestores:
    ...
    def test_env_restored_after_block(self):
        import os
        prev = os.environ.get("PHYLANX_NETWORK")
        with override_network(NETWORK_MAINNET_BETA, mainnet_ok=True):
            assert os.environ["PHYLANX_NETWORK"] == NETWORK_MAINNET_BETA
            assert os.environ["PHYLANX_MAINNET_OK"] == "1"
        assert os.environ.get("PHYLANX_NETWORK") == prev
        assert "PHYLANX_MAINNET_OK" not in os.environ
