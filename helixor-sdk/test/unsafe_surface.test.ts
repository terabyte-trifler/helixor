...
//
// Pins the @helixor/sdk safe-by-default partition. The default entry point
// MUST NOT export raw cert-reader primitives (HelixorClient,
// HelixorChainClient); those live at `@helixor/sdk/unsafe`.
//
...
test("default entry exposes every required safe surface", () => {
  for (const name of REQUIRED_SAFE_EXPORTS) {
    assert.ok(
      ...
      `default @helixor/sdk no longer exports ${name}`,
    );
  }
});
...
test("default entry does NOT expose raw cert-reader primitives", () => {
  for (const name of FORBIDDEN_FROM_DEFAULT) {
    assert.strictEqual(
      ...
      undefined,
      `default @helixor/sdk MUST NOT export ${name} — that lives at @helixor/sdk/unsafe`,
    );
  }
});
...
test("/unsafe exposes the raw cert-reader primitives", () => {
  for (const name of REQUIRED_UNSAFE_EXPORTS) {
    assert.ok(
      ...
      `@helixor/sdk/unsafe no longer exports ${name}`,
    );
  }
});
