...
sanctioned construction site for a mainnet indexer; it:
  ...
  1. Parses `HELIXOR_GEYSER_ENDPOINTS` (comma-separated
     `name:endpoint_url:token_env_var` triples).
  2. When `HELIXOR_SOLANA_CLUSTER` is `mainnet` / `mainnet-beta`,
     requires:
...
CLUSTER_ENV = "HELIXOR_SOLANA_CLUSTER"
...
ENDPOINTS_ENV = "HELIXOR_GEYSER_ENDPOINTS"
...
@dataclass(frozen=True, slots=True)
class _EndpointSpec:
    """Parsed form of one `HELIXOR_GEYSER_ENDPOINTS` entry."""
    label:         str
    endpoint:      str
    token_env_var: str = ""
...
def build_production_geyser_config(
    *,
    ...
    ----------
    cluster
        Override for `HELIXOR_SOLANA_CLUSTER`. Tests pass it explicitly;
        deployment leaves it None so the env var is read.
    endpoint_spec
        Override for `HELIXOR_GEYSER_ENDPOINTS`. Same rationale.
    token_lookup
