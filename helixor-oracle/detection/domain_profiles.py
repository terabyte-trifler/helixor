...
domain, over the five txtype categories Helixor's feature extractor uses:
...
Helixor does not punish an agent for operating in a domain Helixor has no
profile for.
"""
...
def domain_profile(domain: str) -> tuple[float, float, float, float, float] | None:
    """
    The expected txtype distribution for `domain`, or None if Helixor has
