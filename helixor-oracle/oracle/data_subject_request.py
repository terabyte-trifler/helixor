...
that path for the Helixor oracle.
...
# `HELIXOR_DB_DSN`. It writes the audit event to stdout (operator
...
def _build_argparser():
    ...
    parser = argparse.ArgumentParser(
        prog="dsar",
        description="Helixor DP-1 DSAR (data subject access + erasure) CLI",
    )
...
def _connect_from_env():
    """Build a `Psycopg3Connection` from `HELIXOR_DB_DSN`. Deferred
    import so `psycopg` is not a hard dep of this module."""
    import os
    dsn = os.environ.get("HELIXOR_DB_DSN")
    if not dsn:
        raise SystemExit(
            "error: HELIXOR_DB_DSN env var must be set for DSAR CLI"
        )
