// =============================================================================
// HELIXOR_AUTO_PAUSE_STATUS action (Day 42)
//
...
export const autoPauseStatusAction: Action = {
  name: "HELIXOR_AUTO_PAUSE_STATUS",
  description: "Report whether the runtime is currently auto-paused by the Helixor trust score.",
  similes: [
    "are you paused",
    "is the agent paused",
    "is the runtime halted",
    "helixor pause status",
  ...
  examples: [
    [
      { user: "{{user1}}", content: { text: "Are you paused?" } },
      {
        user: "{{agentName}}",
        content: {
          text: "I'm active. Helixor reports me as GREEN at 850/1000.",
          action: "HELIXOR_AUTO_PAUSE_STATUS",
        },
      },
    ],
  ],
  ...
  handler: async (
    ...
    if (!snap.paused) {
      ...
      text = sc
        ? `I'm active. Helixor reports me as ${sc.alert} at ${sc.score}/1000.`
        : "I'm active. No Helixor score has been fetched yet this session.";
    } else {
    ...
    if (callback) {
      await callback({ text, action: "HELIXOR_AUTO_PAUSE_STATUS" });
    }
    return true;
  },
};
