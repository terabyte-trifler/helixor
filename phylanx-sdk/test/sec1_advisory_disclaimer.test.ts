...
// from `phylanx-oracle/oracle/securities_compliance.py` (ADVISORY_DISCLAIMER).
...
test("ADVISORY_DISCLAIMER tells the consumer the decision is theirs alone", () => {
  ...
  assert.match(ADVISORY_DISCLAIMER, /MUST NOT treat a Phylanx cert score as a recommendation/);
});
...
test("ADVISORY_DISCLAIMER export name matches the audit pin", () => {
  ...
  assert.ok(
    /Phylanx cert scores are technical trust signals/.test(ADVISORY_DISCLAIMER),
