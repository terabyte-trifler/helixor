...
test("domain tag is exactly 'phylanx-epoch-advance'", () => {
  assert.deepStrictEqual(
    ADVANCE_EPOCH_DOMAIN_TAG,
    Buffer.from("phylanx-epoch-advance", "utf-8"),
  );
...
test("domain tag is distinct from cert and challenge tags", () => {
  ...
  assert.notDeepStrictEqual(
    ADVANCE_EPOCH_DOMAIN_TAG,
    Buffer.from("phylanx-cert-v1", "utf-8"),
  );
  assert.notDeepStrictEqual(
    ADVANCE_EPOCH_DOMAIN_TAG,
    Buffer.from("phylanx-aw01-ext-challenge", "utf-8"),
  );
});
...
function manualDigest(
  ...
  let o = 0;
  Buffer.from("phylanx-epoch-advance", "utf-8").copy(buf, o); o += 21;
