...
let db: DbHandle;
let client: PhylanxClient;
const cleanups: string[] = [];

beforeAll(async () => {
  db = await openDb(env);
  client = new PhylanxClient({ apiBase: env.apiUrl, cacheTtlMs: 0 });
}, 60_000);
...
describeIf("Failure modes — clean error mapping", () => {
  ...
  it("malformed pubkey → InvalidAgentWalletError (no network call)", async () => {
    const c = new PhylanxClient({ apiBase: env.apiUrl });
...
describeIf("Failure modes — concurrent reads", () => {
  ...
  it("100 parallel reads return the same score", async () => {
    ...
    const c = new PhylanxClient({ apiBase: env.apiUrl, cacheTtlMs: 0 });
