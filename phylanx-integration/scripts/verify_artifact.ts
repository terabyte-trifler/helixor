...
// program at PHYLANX_PROGRAM_ID byte-matches the .so artifact we built and
// tested. Otherwise we have no idea what we deployed.
//
// Workflow:
//   1. Build: anchor build (in phylanx-programs)
...
async function main() {
  ...
  const programsDir = process.env.PHYLANX_PROGRAMS_DIR ?? "../phylanx-programs";
  const localSoPath = path.join(programsDir, "target", "deploy", "health_oracle.so");

  console.log("");
  console.log("╔════════════════════════════════════════════════════════════╗");
  console.log("║  Phylanx — Artifact Verification                          ║");
  ...
  try {
    ...
    console.log("    Run `anchor build` in phylanx-programs first.");
    process.exit(1);
  }
