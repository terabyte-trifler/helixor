# phylanx-integration — Day 13

> **The merge gate.**
>
> Hardening checks + cross-layer integration tests + regression suite.
> CI runs this from the root workflow `.github/workflows/integration.yml`
> (nightly + manual dispatch, gated on the `PHYLANX_DEVNET_RPC_URL`
> secret); nothing should reach Day 14 (devnet validation) until this gate
> is green. The fast unit / Rust / SDK suites that need no devnet run on
> every PR via the root `.github/workflows/ci.yml`.
...
└── (CI lives at the repo root: ../.github/workflows/integration.yml)
```
...
| No CI gate | root `.github/workflows/integration.yml` runs `harden_all` (nightly + manual; PR-fast suites run via `ci.yml`) |
...
4. `PHYLANX_PROGRAM_ID=<mainnet>` `PHYLANX_SOLANA_RPC_URL=<mainnet>` `npm run verify:artifact`
...
*Phylanx MVP · Day 13 complete · Next: Day 14 — 48h devnet validation with 5 agents.*
