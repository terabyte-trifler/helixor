...
fn non_monotonic_nonce_error_code_pinned() {
    ...
    assert_eq!(HelixorError::NonMonotonicNonce as u32, 6020);
}
...
fn nonce_not_strict_successor_error_code_pinned() {
    ...
    assert_eq!(HelixorError::NonceNotStrictSuccessor as u32, 6025);
}
...
fn nonce_space_exhausted_error_code_pinned() {
    ...
    assert_eq!(HelixorError::NonceSpaceExhausted as u32, 6026);
}
...
fn first_commit_at_nonce_two_rejected_as_jump() {
    ...
    assert!(res.is_err());
    assert!(err_matches(res.unwrap_err(), HelixorError::NonceNotStrictSuccessor));
}
...
fn first_commit_at_nonce_zero_rejected_as_rollback() {
    ...
    assert!(res.is_err());
    assert!(err_matches(res.unwrap_err(), HelixorError::NonMonotonicNonce));
}
...
fn stored_at_u64_max_can_never_succeed() {
    ...
    for new in [0u64, 1, u64::MAX / 2, u64::MAX - 1, u64::MAX] {
        ...
        assert!(
            err_matches(res.unwrap_err(), HelixorError::NonMonotonicNonce),
