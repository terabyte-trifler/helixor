# phylanx-e2e — End-to-end loop validation
...
| `Program ... not found on RPC` | Program not deployed | `cd phylanx-programs && anchor deploy --provider.cluster devnet` |
...
| Smoke test fails on `/docs` | API not running | `cd phylanx-oracle && uvicorn api.main:app --port 8001` |
| `OracleConfig not initialized` from epoch_runner | Day 7 setup not run | `cd phylanx-programs && ts-node scripts/initialize_oracle_config.ts` |
...
*Phylanx MVP · Day 10 complete · The full loop runs end-to-end in 3 minutes.*
