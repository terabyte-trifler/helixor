...
// program ID is stable. Refuse to run if PHYLANX_PROGRAM_ID matches a
...
export function loadEnv(): E2EEnv {
  ...
  const apiUrl    = process.env.PHYLANX_API_URL!;
  const rpcUrl    = process.env.SOLANA_RPC_URL!;
  const programId = process.env.PHYLANX_PROGRAM_ID!;
  ...
  if (FORBIDDEN_PROGRAM_IDS.includes(programId)) {
    throw new EnvError(
      `PHYLANX_PROGRAM_ID is the mainnet program. E2E tests must run on devnet/localnet.`,
    );
  }
  ...
  try {
    ...
    throw new EnvError(`PHYLANX_PROGRAM_ID is not a valid base58 pubkey: ${programId}`);
  }

  let consumerProgramPk: PublicKey | null = null;
  if (process.env.PHYLANX_CONSUMER_PROGRAM_ID) {
    try {
      consumerProgramPk = new PublicKey(process.env.PHYLANX_CONSUMER_PROGRAM_ID);
    } catch {
      throw new EnvError(`PHYLANX_CONSUMER_PROGRAM_ID is not a valid base58 pubkey.`);
    }
  }
