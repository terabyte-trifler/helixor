...
# The Day-14 synthetic agent profiles live in helixor-oracle/tests; make
# them importable without coupling helixor-api's tests/ layout to the
...
def _load_agent_profiles():
    """Load helixor-oracle's tests/oracle/agent_profiles.py as a free
    module. We cannot `import tests.oracle.agent_profiles` because the
    `tests` package on sys.path is the helixor-api one — same name,
    ...
    spec = importlib.util.spec_from_file_location(
        "_helixor_oracle_agent_profiles", path,
    )
