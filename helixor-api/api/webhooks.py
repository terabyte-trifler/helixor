...
---------------------------------
helixor-api has no scheduler substrate (no Celery, no APScheduler).
...
logger = logging.getLogger("helixor.api.webhooks")
...
SIGNATURE_HEADER: str = "X-Helixor-Webhook-Signature"
EVENT_HEADER:     str = "X-Helixor-Webhook-Event"
...
def compute_signature(secret: str, body_bytes: bytes) -> str:
    ...
    re-computes to verify a delivery is from Helixor and not a forgery."""
    mac = hmac.new(
        secret.encode("utf-8"),
        body_bytes,
        hashlib.sha256,
