...
without a TimescaleDB instance:
    ...
    PYTHONPATH=.:../helixor-oracle HELIXOR_SYNTHETIC_AGENTS=50 \
...
logger = logging.getLogger("helixor.api.synthetic")


try:
    _VERDICT = enforce_network_guard(service="helixor-api-synthetic")
...
def build_app():
    count = int(os.environ.get("HELIXOR_SYNTHETIC_AGENTS", "50"))
    score_repo, byz_repo, cluster_repo, explanations = build_synthetic_repos(
        count=count,
    )
    report_path = Path(os.environ.get(
        "HELIXOR_SYNTHETIC_REPORT",
        "/tmp/helixor_synthetic_report.json",
    ))
...
def main() -> int:
    ...
    host = os.environ.get("HELIXOR_API_HOST", "127.0.0.1")
    logger.info("synthetic helixor-api starting on %s:%d", host, port)
