...
pub fn check_strict_successor_nonce(
    ...
    require!(
        ...
        PhylanxError::NonMonotonicNonce,
    );
    let expected = stored_nonce
        .checked_add(1)
        .ok_or(PhylanxError::NonceSpaceExhausted)?;
    require!(
        new_nonce == expected,
        PhylanxError::NonceNotStrictSuccessor,
    );
    Ok(())
}
...
pub fn check_oracle_commit_cooldown(
    ...
    let earliest = previous_committed_at
        ...
        .ok_or(PhylanxError::BaselineTimestampOverflow)?;
    require!(now >= earliest, PhylanxError::OracleCommitCooldownActive);
    Ok(())
}
...
pub struct CommitBaseline<'info> {
    ...
    #[account(
        ...
        constraint = agent_registration.active                                  @ PhylanxError::AgentInactive,
        constraint = agent_registration.layout_version == AgentRegistration::CURRENT_LAYOUT_VERSION
            @ PhylanxError::LayoutMigrationRequired,
    )]
...
pub fn handler(ctx: Context<CommitBaseline>, args: CommitBaselineArgs) -> Result<()> {
    ...
    match args.committer_kind {
        CommitterKind::Oracle => {
            require_keys_eq!(
                signer.key(),
                config.oracle_node,
                PhylanxError::NotOracleAuthority
            );
        }
        CommitterKind::Owner => {
            require_keys_eq!(
    ...
    require!(!args.payload.is_empty(), PhylanxError::BaselinePayloadEmpty);
    require!(
        args.payload.len() <= MAX_BASELINE_PAYLOAD_LEN,
        PhylanxError::BaselinePayloadTooLarge,
    );
    let computed_hash = hashv(&[&args.payload]).to_bytes();
    require!(
        computed_hash == args.baseline_hash,
        PhylanxError::BaselinePayloadHashMismatch,
    );
