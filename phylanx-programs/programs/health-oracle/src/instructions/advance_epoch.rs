...
// DOMAIN SEPARATION
// -----------------
// The advance digest uses the distinct tag `b"phylanx-epoch-advance"`,
...
//
// Distinct from `phylanx-cert-v1` (cert signing) and
// `phylanx-aw01-ext-challenge` (challenge attestations) so an honest
// cluster signature can NEVER be lifted from one purpose to another.
// -----------------------------------------------------------------------------
pub const ADVANCE_EPOCH_DOMAIN_TAG: &[u8] = b"phylanx-epoch-advance";
...
///
/// Layout (fixed, public):
///   "phylanx-epoch-advance"   (21 bytes) — domain separator
...
fn parse_ed25519_ix(ix: &Instruction) -> Result<PrecompileRecord> {
    require!(
        ...
        PhylanxError::MalformedAdvanceEd25519Instruction,
    );
    require!(
    ...
    require!(
        ...
        PhylanxError::AdvanceCrossInstructionReference,
    );
    require!(
        msg_size == ED25519_MESSAGE_LEN,
        PhylanxError::WrongAdvanceDigestLength,
    );
    require!(
...
pub fn verify_cluster_threshold(
    ...
    require!(
        ...
        PhylanxError::WrongAdvanceInstructionsSysvar,
    );
    ...
    require!(
        count >= required,
        PhylanxError::InsufficientAdvanceAttestations,
    );
    Ok(count)
}
...
pub fn propose_handler(ctx: Context<AdvanceEpoch>) -> Result<()> {
    ...
    require!(may_advance, PhylanxError::EpochNotElapsed);
    ...
    require!(
        overwrite_ok,
        PhylanxError::PendingAdvanceAlreadyInFlight,
    );

    let target_epoch = current_epoch
        .checked_add(1)
        .ok_or(PhylanxError::EpochCounterOverflow)?;
    ...
    let (by_fallback, attester_count) = match tier1 {
        ...
        Err(_)    => {
            ...
            require!(
                ...
                PhylanxError::NotAuthorisedAdvancer,
            );
            (true, 1u8)
        }
    };
...
pub fn finalize_handler(ctx: Context<FinalizeAdvanceEpoch>) -> Result<()> {
    ...
    require!(
        ...
        PhylanxError::PendingAdvanceTargetDrift,
    );
...
mod tests {
    ...
    fn digest_uses_distinct_domain_tag() {
        ...
        assert_eq!(ADVANCE_EPOCH_DOMAIN_TAG, b"phylanx-epoch-advance");
        assert_ne!(ADVANCE_EPOCH_DOMAIN_TAG, b"phylanx-cert-v1");
        assert_ne!(ADVANCE_EPOCH_DOMAIN_TAG, b"phylanx-aw01-ext-challenge");
    }
