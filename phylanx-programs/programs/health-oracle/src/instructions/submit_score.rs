...
pub struct SubmitScore<'info> {
    ...
    #[account(
        ...
        constraint = agent_registration.baseline_committed
            @ PhylanxError::BaselineNotCommitted,
    )]
    ...
    #[account(
        constraint = baseline_stats.agent_wallet == agent_registration.agent_wallet
            @ PhylanxError::BaselineStatsAgentMismatch,
        constraint = baseline_stats.baseline_hash == agent_registration.baseline_hash
            @ PhylanxError::BaselineStatsHashMismatch,
    )]
...
pub fn handler(
    ...
    let floor_required = escrow_balance_before_deposit
        ...
        .ok_or(PhylanxError::SubmitEscrowBelowFloor)?;
    require!(
        escrow_balance_after_deposit >= floor_required,
        PhylanxError::SubmitEscrowBelowFloor,
    );
