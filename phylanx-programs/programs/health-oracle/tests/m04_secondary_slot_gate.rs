...
// with PhylanxError attribution. `submit_score` calls it BEFORE the CPI.
...
fn wrong_slot_hashes_sysvar_error_code_pinned() {
    ...
    assert_eq!(PhylanxError::WrongSlotHashesSysvar as u32, 6090);
}

#[test]
fn missing_slot_anchor_error_code_pinned() {
    assert_eq!(PhylanxError::MissingSlotAnchor as u32, 6091);
}

#[test]
fn slot_anchor_too_old_error_code_pinned() {
    assert_eq!(PhylanxError::SlotAnchorTooOld as u32, 6092);
}

#[test]
fn slot_anchor_hash_mismatch_error_code_pinned() {
    assert_eq!(PhylanxError::SlotAnchorHashMismatch as u32, 6093);
}
...
fn err_matches(err: AnchorError, code: PhylanxError) -> bool {
...
fn zero_anchor_hash_rejected_as_missing() {
    ...
    with_account_info(key, &mut data, |info| {
        ...
        assert!(err_matches(err, PhylanxError::MissingSlotAnchor));
    });
}
...
fn foreign_account_rejected_as_wrong_sysvar() {
    ...
    with_account_info(foreign, &mut data, |info| {
        ...
        assert!(err_matches(err, PhylanxError::WrongSlotHashesSysvar));
    });
}
...
fn slot_present_but_hash_wrong_rejected_as_hash_mismatch() {
    ...
    with_account_info(key, &mut data, |info| {
        ...
        assert!(err_matches(err, PhylanxError::SlotAnchorHashMismatch));
    });
}
...
fn slot_below_window_rejected_as_too_old() {
    ...
    with_account_info(key, &mut data, |info| {
        ...
        assert!(err_matches(err, PhylanxError::SlotAnchorTooOld));
    });
}
