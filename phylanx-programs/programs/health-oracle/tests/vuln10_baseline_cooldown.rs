...
fn oracle_same_second_rotation_blocked() {
    ...
    assert!(err_matches(err, PhylanxError::OracleCommitCooldownActive));
}
...
fn overflow_in_cooldown_addition_returns_typed_error() {
    ...
    assert!(err_matches(err, PhylanxError::BaselineTimestampOverflow));
}
...
fn vuln10_error_codes_are_stable() {
    ...
    assert_eq!(PhylanxError::OracleCommitCooldownActive as u32, 6023);
    assert_eq!(PhylanxError::BaselineTimestampOverflow  as u32, 6024);
}
