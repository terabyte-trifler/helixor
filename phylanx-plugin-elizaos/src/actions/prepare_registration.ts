// =============================================================================
// PHYLANX_PREPARE_REGISTRATION action.
//
...
export const prepareRegistrationAction: Action = {
  name: "PHYLANX_PREPARE_REGISTRATION",
  description: "Build an unsigned register_agent transaction the operator's wallet must sign.",
  similes: ["register me with phylanx", "sign me up for phylanx", "enroll in phylanx"],
  examples: [],
    ...
    try {
      ...
      if (callback) {
        await callback({
          ...
          action: "PHYLANX_PREPARE_REGISTRATION",
      ...
      if (callback) {
        await callback({
          text: `Failed to prepare Phylanx registration: ${(err as Error).message}`,
          action: "PHYLANX_PREPARE_REGISTRATION",
        });
      }
      return false;
