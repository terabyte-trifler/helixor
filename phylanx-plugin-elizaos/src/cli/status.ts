#!/usr/bin/env node
// =============================================================================
// CLI: npx @elizaos/plugin-phylanx status
//
...
async function main(): Promise<number> {
  const apiUrl = process.env.PHYLANX_API_URL ?? "https://api.phylanx.xyz";
  const apiKey = process.env.PHYLANX_API_KEY;

  if (!apiKey) {
    console.error(`${RED}error:${NC} PHYLANX_API_KEY is not set in environment.`);
    console.error(`Set it in your .env, then re-run.`);
    return 2;
  }

  console.log("");
  console.log(`${BOLD}Phylanx plugin status${NC}  ${DIM}(v${PLUGIN_VERSION}, ${apiUrl})${NC}`);
  console.log("");
  ...
  if (response.status === 401) {
    ...
    console.error(`  Reach out to the Phylanx team if you think this is wrong.`);
    return 1;
  }
  if (!response.ok) {
  ...
  if (data.plugin_initialized_count > 0 && data.integrations.length > 0) {
    console.log(`  ${GREEN}✓ Phylanx plugin is installed + reporting telemetry${NC}`);
    console.log("");
    console.log(`  ${DIM}Paste this confirmation into your Discord/Telegram:${NC}`);
    console.log("");
    console.log(`     "Phylanx plugin v${PLUGIN_VERSION} is running for ${data.organization ?? "us"}.`);
  ...
  console.log(`    1. PHYLANX_API_KEY is set in your agent's .env`);
