// =============================================================================
// src/telemetry/beacon.ts — sends plugin events to Phylanx API.
//
...
export type BeaconEventType =
  ...
  | "agent_deactivated"
  | "plugin_shutdown"
  // VULN-12 — fail-closed-with-cache decisions. Operators alert on a
...
export class TelemetryBeaconClient {
  ...
  constructor(cfg: BeaconClientConfig) {
    ...
    if (!f) throw new Error("[Phylanx.telemetry] no fetch available");
  ...
  private async send(beacon: BeaconPayload): Promise<boolean> {
    const headers: Record<string, string> = {
      ...
      "User-Agent":   `@elizaos/plugin-phylanx/${PLUGIN_VERSION}`,
    };
    if (this.cfg.apiKey) {
