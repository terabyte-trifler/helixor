...
// runtime + emits the runtime event `phylanx:auto_paused` so downstream
...
export const autoPauseEvaluator: Evaluator = {
  ...
  handler: async (
    ...
    if (!snap.paused) return "phylanx:active";

    const rt = runtime as { emit?: (e: string, payload: unknown) => void };
    rt.emit?.("phylanx:auto_paused", {
      reason:        snap.reason,
    ...
    return `phylanx:auto_paused:${snap.reason ?? "UNKNOWN"}`;
  },
};
