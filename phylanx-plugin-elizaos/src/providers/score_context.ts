// =============================================================================
// score_context provider — injects current Phylanx score into the agent's
// prompt context.
//
// Why: the agent should KNOW its own score so it can mention it
// conversationally ("My current Phylanx trust score is 850, GREEN.") and
...
export const scoreContextProvider: Provider = {
  get: async (runtime: IAgentRuntime, _message: Memory, _state?: State): Promise<string> => {
    ...
    const lines = [
      `Phylanx trust score: ${score.score}/1000 (${score.alert}).`,
    ...
    if (score.score < config.minScore) {
      lines.push(
        ...
        `financial actions will be blocked by PHYLANX_TRUST_GATE.`,
      );
    }
