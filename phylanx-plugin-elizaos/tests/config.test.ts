...
import { PhylanxConfigError, loadConfig } from "../src/config";
...
describe("loadConfig", () => {
  ...
  it("accepts mode=warn", () => {
    const cfg = loadConfig(makeRuntime({ settings: { PHYLANX_MODE: "warn" } }) as any);
    expect(cfg.mode).toBe("warn");
  });

  it("accepts mode=observe", () => {
    const cfg = loadConfig(makeRuntime({ settings: { PHYLANX_MODE: "observe" } }) as any);
    expect(cfg.mode).toBe("observe");
  });

  it("rejects invalid mode", () => {
    expect(() =>
      loadConfig(makeRuntime({ settings: { PHYLANX_MODE: "bogus" } }) as any),
    ).toThrow(PhylanxConfigError);
  });

  it("accepts fail_mode=open", () => {
    const cfg = loadConfig(makeRuntime({ settings: { PHYLANX_FAIL_MODE: "open" } }) as any);
    expect(cfg.failMode).toBe("open");
  });

  it("rejects invalid fail_mode", () => {
    expect(() =>
      loadConfig(makeRuntime({ settings: { PHYLANX_FAIL_MODE: "wat" } }) as any),
    ).toThrow(/must be 'closed' or 'open'/);
  });

  it("telemetry endpoint defaults to {api_url}/telemetry/beacon", () => {
    const cfg = loadConfig(makeRuntime({
      settings: { PHYLANX_API_URL: "http://example.com" },
    }) as any);
  ...
  it("telemetry can be disabled", () => {
    const cfg = loadConfig(makeRuntime({
      settings: { PHYLANX_TELEMETRY_DISABLED: "true" },
    }) as any);
  ...
  it("telemetry endpoint can be overridden", () => {
    const cfg = loadConfig(makeRuntime({
      settings: { PHYLANX_TELEMETRY_ENDPOINT: "http://my-collector.test/v1" },
    }) as any);
  ...
  it("apiKey is forwarded when set", () => {
    const cfg = loadConfig(makeRuntime({
      settings: { PHYLANX_API_KEY: "hxop_secret123" },
    }) as any);
