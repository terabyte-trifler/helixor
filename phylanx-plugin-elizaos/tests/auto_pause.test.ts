...
//      AND emits the phylanx:auto_paused runtime event.
...
//   6. PHYLANX_AUTO_PAUSE=false disables the whole machine.
...
describe("AUTO_PAUSE — trigger conditions", () => {
  ...
  it("RED score trips pause and emits auto_paused beacon", async () => {
    ...
    const runtime = makeRuntime({
      settings: { PHYLANX_TELEMETRY: "true", PHYLANX_TELEMETRY_DISABLED: "false" },
    });
  ...
  it("sub-min score trips pause even with a YELLOW alert", async () => {
    restore = withBeaconCapture();
    const runtime = makeRuntime({ settings: { PHYLANX_MIN_SCORE: "700" } });
    const state = getOrInitState(runtime as any, loadConfig(runtime as any));
...
describe("AUTO_PAUSE — hysteresis recovery", () => {
  ...
  it("single GREEN after pause does NOT auto-resume (needs autoResumeEpochs)", async () => {
    restore = withBeaconCapture();
    const runtime = makeRuntime();   // default PHYLANX_RECOVER_EPOCHS = 2
    const state = getOrInitState(runtime as any, loadConfig(runtime as any));
  ...
  it("PHYLANX_RECOVER_EPOCHS=1 resumes immediately on first healthy score", () => {
    restore = withBeaconCapture();
    const runtime = makeRuntime({ settings: { PHYLANX_RECOVER_EPOCHS: "1" } });
    const state = getOrInitState(runtime as any, loadConfig(runtime as any));
...
describe("AUTO_PAUSE — evaluator surface", () => {
  ...
  it("evaluator returns phylanx:active when not paused", async () => {
    restore = withBeaconCapture();
    const runtime = makeRuntime();
    const r = await autoPauseEvaluator.handler(runtime as any, {} as any);
    expect(r).toBe("phylanx:active");
  });

  it("evaluator emits phylanx:auto_paused runtime event when paused", async () => {
    restore = withBeaconCapture();
    const runtime = makeRuntime();
    const state = getOrInitState(runtime as any, loadConfig(runtime as any));
    state.recordScore(ts({ score: 100, alert: "RED" }));
    ...
    const r = await autoPauseEvaluator.handler(runtime as any, {} as any);
    expect(r).toBe("phylanx:auto_paused:ALERT_RED");

    const ev = runtime._events.find(e => e.name === "phylanx:auto_paused");
    expect(ev).toBeDefined();
    expect((ev!.payload as any).reason).toBe("ALERT_RED");
    expect((ev!.payload as any).needsStreak).toBe(2);
  });

  it("PHYLANX_AUTO_PAUSE=false disables the whole machine", async () => {
    restore = withBeaconCapture();
    const runtime = makeRuntime({ settings: { PHYLANX_AUTO_PAUSE: "false" } });
    const state = getOrInitState(runtime as any, loadConfig(runtime as any));
