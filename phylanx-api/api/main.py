...
service. It:
  ...
  1. ENFORCES THE NETWORK GUARD at module-import time. If
     PHYLANX_NETWORK=mainnet-beta and PHYLANX_MAINNET_OK is not set,
  ...
  2. Constructs the production-shape repositories. Today those are
     ...
     existing phylanx-oracle/db/timescale_repo.py connection. Until the
...
--------------------
The systemd unit (launch/deploy/systemd/phylanx-api.service) sets:

    PHYLANX_NETWORK       = devnet | mainnet-beta | ...
    PHYLANX_MAINNET_OK    = 1  (only when mainnet is intended)
    PHYLANX_API_PORT      = 8080
    PHYLANX_METRICS_PORT  = 9090
...
logger = logging.getLogger("phylanx.api.main")
...
# the shared `phylanx-oracle` package on PYTHONPATH; both production and
...
try:
    ...
    print(
        ...
        f"phylanx-oracle must be on PYTHONPATH.",
        file=sys.stderr,
    )
    sys.exit(2)
...
def _enforce_or_exit() -> object:
    ...
    try:
        return enforce_network_guard(service="phylanx-api")
...
def _build_repos():
    ...
    TimescaleDB adapter layer:
      ...
      phylanx-api/api/_timescale.py::TimescaleScoreRepo
      phylanx-api/api/_timescale.py::TimescaleByzantineRepo
      phylanx-api/api/_timescale.py::TimescaleClusterHealthRepo

    Each adapts the existing phylanx-oracle/db/timescale_repo.py
    ...
    logger.warning(
        ...
        "in-memory repos. Reads will return 404. See phylanx-api/README.md "
...
def build_app():
    ...
    # An empty PHYLANX_API_KEYS is a legitimate (if locked-down) state —
    ...
    if not api_keys and _VERDICT.is_production:
        logger.warning(
            "VULN-09: production start with no PHYLANX_API_KEYS — "
            "operational endpoints (/health/cluster, /byzantine/*, "
            "/challenges) will reject every caller. Set PHYLANX_API_KEYS "
    ...
    # PHYLANX_WEBHOOKS means no partner has subscribed yet — the
...
def main() -> int:
    ...
    host    = os.environ.get("PHYLANX_API_HOST", "0.0.0.0")
    workers = int(os.environ.get("PHYLANX_API_WORKERS", "1"))

    logger.info(
        "phylanx-api %s starting on %s:%d (network=%s, production=%s, workers=%d)",
