...
Every HTTP response in phylanx-api is one of these Pydantic models. The
...
the SDK's `EpochScore` exposes (phylanx-sdk/src/types.ts) — clients can
...
class EvidenceVerificationRecipe(BaseModel):
    ...
    re-derive every Phylanx canonical hash without trusting the
    server's serialiser.
    """
