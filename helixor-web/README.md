...
The Helixor consumer + integrator-facing site. Next.js 15 App Router,
...
│   └── charts/                # HistorySpark
├── lib/
│   ├── api.ts                 # Fetch client → helixor-api
...
├── types/
│   └── api.ts                 # Wire types mirroring helixor-api/api/schemas.py
...
3. Every page that shows data shows the *time it was computed* and a
   link to the on-chain proof. Helixor's product is auditability.
