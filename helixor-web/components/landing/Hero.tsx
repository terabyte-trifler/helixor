...
export function Hero() {
  ...
  return (
    <section ref={sectionRef} className="relative bg-accent text-ink-0 overflow-hidden">
      {/* Cursor-reactive tone-on-tone glyph */}
      <div
        ref={glyphRef}
        aria-hidden
        className="absolute -right-10 top-1/2 -translate-y-1/2 font-mono select-none pointer-events-none text-black/10 text-[26rem] leading-none tracking-[-0.08em] hidden lg:block will-change-transform"
      >
        ///
      </div>
      ...
      <div ref={contentRef} className="mx-auto max-w-7xl px-6 lg:px-10 pt-14 pb-16 lg:pt-24 lg:pb-20 relative will-change-transform">
        <div className="max-w-3xl">
          <div className="reveal" style={{ "--d": "0s" } as React.CSSProperties}>
            <span className="inline-flex items-center gap-2 h-7 px-3.5 rounded-[3px] border border-black/40 text-black/80 font-mono text-[11px] tracking-eyebrow uppercase">
              <span className="h-1.5 w-1.5 rounded-full bg-black/70" aria-hidden />
