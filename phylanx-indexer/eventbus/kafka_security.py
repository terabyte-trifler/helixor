...
internal address), any producer can publish to any topic. Phylanx
...
config is acceptable for the current PHYLANX_NETWORK.

The rules:

  * PRODUCTION (mainnet-beta) — REFUSE plaintext. The protocol must
    be one of {SSL, SASL_SSL}, OR the operator must set
    PHYLANX_KAFKA_PLAINTEXT_OK=1 to make the decision conscious and
...
logger = logging.getLogger("phylanx.eventbus.kafka_security")
...
ENV_PLAINTEXT_OK      = "PHYLANX_KAFKA_PLAINTEXT_OK"
...
def evaluate(env: dict[str, str] | None = None) -> KafkaSecurityVerdict:
    ...
    network = _strip(env.get("PHYLANX_NETWORK")) or "localnet"
