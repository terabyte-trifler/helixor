...
```bash
PHYLANX_VALIDATION_INTERVAL_MS=600000 npm run run-full
# 12 ticks × 10min = 2h
```
...
│   (CI lives at the repo root: ../.github/workflows/validation.yml —
│    weekly compressed validation, gated on the devnet secret)
├── package.json
├── tsconfig.json
├── .env.example
└── README.md
...
If the failure is a real bug → file a regression test in `phylanx-integration/tests/integration/regressions/` (Day 13's structure) before fixing.
...
*Phylanx MVP · Day 14 complete · Next: Day 15 — mainnet deployment with verified artifact + first paying partner.*
