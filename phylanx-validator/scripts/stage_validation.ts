...
async function main(): Promise<number> {
  console.log("");
  console.log("╔════════════════════════════════════════════════════════════╗");
  console.log("║  Phylanx — Day 14 Devnet Validation: STAGE                 ║");
  console.log("║  5 agents · 48h continuous · machine-checkable verdict     ║");
  console.log("╚════════════════════════════════════════════════════════════╝");
  console.log("");

  const env = loadEnv();
  const durationHours = Number(process.env.PHYLANX_VALIDATION_DURATION_HOURS ?? DEFAULT_DURATION_HOURS);
  const snapshotIntervalMin = Number(
    process.env.PHYLANX_VALIDATION_SNAPSHOT_INTERVAL_MIN ?? DEFAULT_SNAPSHOT_INTERVAL_MIN,
  );
  ...
  if (process.env.PHYLANX_VALIDATION_FRESH_START === "1") {
