...
import { PhylanxClient } from "@phylanx/client/unsafe";
...
async function main(): Promise<number> {
  ...
  let runId = process.env.PHYLANX_VALIDATION_RUN_ID;
  if (!runId) {
  ...
  const intervalMs = Number(process.env.PHYLANX_VALIDATION_INTERVAL_MS ?? 1_800_000); // 30min default

  console.log("");
  console.log(`Phylanx — Day 14 Validation Monitor (runId=${runId}, interval=${intervalMs/60000}min)`);
  console.log("");

  const client = new PhylanxClient({ apiBase: env.apiUrl, cacheTtlMs: 0 });
