...
export function loadEnv(): ValidationEnv {
  ...
  try { programPk = new PublicKey(process.env.PHYLANX_PROGRAM_ID!); }
  catch { throw new EnvError(`PHYLANX_PROGRAM_ID is not valid base58.`); }
  ...
  return {
    ...
    solanaRpcUrl: rpcUrl,
    oracleDir:    process.env.PHYLANX_ORACLE_DIR ?? "../phylanx-oracle",
    ownerKeypairPath: process.env.PHYLANX_VALIDATION_OWNER_KEYPAIR_PATH
      ?? path.join(os.homedir(), ".config", "solana", "id.json"),
    stateDir:     process.env.PHYLANX_VALIDATION_STATE_DIR ?? "./reports",
  };
}
